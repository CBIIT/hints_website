...
function BuildDisplay() {
    ...
    for (var i = 0; i < QuestionDetails_ds.length; i++) {
        ...
        if ((CycleBeingDisplayed == QuestionDetails_ds[i].FK_Cycle)) {
            ...
            if (QuestionDetails_ds[i].VariableName !== null) {
                $("#LIT_variablename").html(QuestionDetails_ds[i].VariableName);
                $('#dv_LIT_variablename_wrapper').show();
            }
            else {
                $('#dv_LIT_variablename_wrapper').hide();
            }
            ...
            if (QuestionDetails_ds[i].Additional_Notes !== null) {
                $("#LIT_Notes").html(QuestionDetails_ds[i].Additional_Notes);
...
function getRawValuesForChartForAllCycles(summaryTypeId) {
    ...
    for (var prop in surveyQuestionData.SurveyCycles) {
     ...
     var cycleDisplayOrder =Number( surveyQuestionData.SurveyCycles[prop].Displayorder);
            ...
            for (var i = 0; i < valuesForChartForCycle[Object.keys(valuesForChartForCycle)[0]].length; i++) {
                valuesForChart.Cycles.push(cycleDisplayText);
                     valuesForChart.CycleDisplayOrder.push(cycleDisplayOrder);
            }
...
function getConfigDataForLineChart(xAxisName) {
    ...
    return {
        ...
        paletteColors: "#b30000,#ffde17,#92278f,#009444,#ec008c,#21409a,#f26522,#262262,#ff5f5f,#8dc63f,#fbb040,#00aeef,#662d91,#006838"
    };
}
...
// *************************************************************
    ...
    //  FOR LINE CHART
        ...
        for (var i = 0; i < preFormattedData.rows.length; i++) {
            let cycleName = preFormattedData.rows[i][preFormattedData.columns[0]];

            if (distinctCycles.indexOf(cycleName) == -1) {
                distinctCycles.push(cycleName);
            }
        }

        let distinctResponseLabels = [];

        for (var i = 0; i < preFormattedData.rows.length; i++) {
            let responseLabel = preFormattedData.rows[i][preFormattedData.columns[2]];

            if (distinctResponseLabels.indexOf(responseLabel) == -1) {
                distinctResponseLabels.push(responseLabel);
            }
        }

        let preFormattedDataForIterating = JSON.parse(JSON.stringify(preFormattedData));

        for (var i = 0; i < distinctCycles.length; i++) {
            let currentCycleName = distinctCycles[i];
            let currentCycleDisplayOrder;
            let distinctResponseLabelsForElimination = distinctResponseLabels.slice();

            for (var ii = 0; ii < preFormattedDataForIterating.rows.length; ii++) {
                let cycleNameForRow = preFormattedDataForIterating.rows[ii][preFormattedDataForIterating.columns[0]];
                let responseLabel = preFormattedDataForIterating.rows[ii][preFormattedDataForIterating.columns[2]];

                if (cycleNameForRow === currentCycleName && distinctResponseLabelsForElimination.indexOf(responseLabel) !== -1) {
                 
currentCycleDisplayOrder = preFormattedDataForIterating.rows[ii][preFormattedDataForIterating.columns[1]];
   distinctResponseLabelsForElimination.splice(distinctResponseLabelsForElimination.indexOf(responseLabel), 1);
                }
            }

            for (var iii = 0; iii < distinctResponseLabelsForElimination.length; iii++) {
                preFormattedData.rows.push({
                    "Cycles": currentCycleName,
                    "CycleDisplayOrder": currentCycleDisplayOrder,
                    "Responses": distinctResponseLabelsForElimination[iii],
                    "Percentages": null
                });
            }
        }

         preFormattedData.rows = preFormattedData.rows.sort(function (firstElement, secondElement) {
            return firstElement.CycleDisplayOrder - secondElement.CycleDisplayOrder || firstElement.Responses.localeCompare(secondElement.Responses);
        });
        ...
        for (var i = 0; i < preFormattedData.rows.length; i++) {
            ...
            var seriesName = preFormattedData.rows[i][preFormattedData.columns[2]];
            ...
            series.data.push({
                value: preFormattedData.rows[i][preFormattedData.columns[3]]
            });
        }
